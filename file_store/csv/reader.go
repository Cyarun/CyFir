// A version of the standard library's csv reader with the following changes:
...
//    for Velociraptor because the CSV writer does not lock the
//    file. The writer may write partial records (not yet terminated
//    by \n) but the reader will refuse to read the record until the
//    final \n is written. This scheme allows simultaneous writers and
//    readers without needing locks.
...
func (r *Reader) readLine() ([]byte, error) {
	...
	// A short line was found, the record is not complete.
	...
	// All lines must be terminated by line separator. If a line
	// is not, then it is not fully written yet.
	n := len(line)
	if r.RequireLineSeperator && n > 0 && line[n-1] != '\n' {
		return nil, &ParseError{StartLine: r.numLine + 1, Line: r.numLine + 1,
			Column: 0, Err: ErrNoLineFeed}
	}

	r.numLine++

	// FIXME: This is buggy because we might be parsing a quoted
	// field here and this will convert the field value from \r\n
	// to \n.
	// Normalize \r\n to \n on all input lines.
...
func (r *Reader) readRecord(dst []string) ([]string, error) {
	...
	if errRead == io.EOF {
		// Revert the byte offset to the start of this record.
		r.ByteOffset = start_offset
		return nil, errRead
	}
